...
const ASPECT_RATIOS = [
  { value: 'Auto', label: 'Auto', icon: 'aspectRatio' },
  { value: '1:1', label: '1:1 Square', icon: 'square' },
  { value: '21:9', label: '21:9 Ultrawide', icon: 'rectangle' },
  { value: '8:1', label: '8:1 Panoramic', icon: 'fullscreen' },
  { value: '4:1', label: '4:1 Banner', icon: 'rectangle' },
  { value: '16:9', label: '16:9 Widescreen', icon: 'rectangle' },
  { value: '9:16', label: '9:16 Social story', icon: 'rectangle', iconStyle: { transform: 'rotate(90deg)' } },
  { value: '1:4', label: '1:4 Vertical banner', icon: 'rectangle', iconStyle: { transform: 'rotate(90deg)' } },
  { value: '1:8', label: '1:8 Vertical panoramic', icon: 'rectangle', iconStyle: { transform: 'rotate(90deg)' } },
  { value: '4:3', label: '4:3 Classic', icon: 'rectangle' },
  { value: '4:5', label: '4:5 Social post', icon: 'rectangle', iconStyle: { transform: 'rotate(90deg)' } },
...
const modePromptPlaceholder = (mode) => {
  if (mode === 'video') return 'Describe your video—use start/end frames and @ to add references'
...
const referenceRoleLabel = (role) => {
  if (role === 'start') return 'Start image'
  if (role === 'end') return 'End image'
  return 'Add media'
}
...
function ModelRow({ model, selected, resolution, onSelect }) {
  ...
  return (
    ...
    >
      ...
      <span className="engine-cost">{cost.unlimited ? 'âˆž' : `${cost.cost || 0}`}</span>
...
function ReferencesBlock({ mode = 'image', styles, selectedStyle, setSelectedStyle, references, uploadReference, uploading }) {
  const [styleOpen, setStyleOpen] = useState(false)
  const mediaInputRef = useRef(null)
  const startInputRef = useRef(null)
  const endInputRef = useRef(null)

  const startReference = references.find(ref => ref.reference_role === 'start') || null
  const endReference = references.find(ref => ref.reference_role === 'end') || null
  const extraReferences = references.filter(ref => !['start', 'end'].includes(ref.reference_role || 'reference'))

  const handlePick = (file, role = 'reference') => {
    if (file) uploadReference(file, role)
  }
  ...
  if (mode === 'video') {
    return (
      <section className="engine-block">
        ...
        <div className="engine-video-reference-grid">
          {[
            { role: 'start', ref: startReference, inputRef: startInputRef },
            { role: 'end', ref: endReference, inputRef: endInputRef },
          ].map(slot => (
            <button
              key={slot.role}
              type="button"
              className={`engine-video-ref-slot${slot.ref ? ' filled' : ''}`}
              disabled={uploading}
              onClick={() => slot.inputRef.current?.click()}
            >
              {slot.ref ? (
                <>
                  <img src={slot.ref.preview_url} alt="" />
                  <span>{referenceRoleLabel(slot.role)}</span>
                  <small>{slot.ref.alias}</small>
                </>
              ) : (
                <>
          ...
          <button
            type="button"
            className="engine-video-ref-add"
        ...
        {extraReferences.length > 0 && (
          <div className="engine-reference-row engine-reference-row--video">
        ...
        <p className="engine-video-hint">
          Start image sets the opening frame and end image sets the closing frame. Add extra media only if you want the model to keep more visual context.
        </p>
        <input
          ref={startInputRef}
          type="file"
          ...
          hidden
          onChange={event => {
            handlePick(event.target.files?.[0], 'start')
            event.target.value = ''
          }}
        />
        <input
          ...
          hidden
          onChange={event => {
            handlePick(event.target.files?.[0], 'end')
            event.target.value = ''
          }}
        />
        <input
          ...
          hidden
          onChange={event => {
            handlePick(event.target.files?.[0], 'reference')
            event.target.value = ''
          }}
        />
      </section>
      ...
      <div className="engine-reference-row">
        ...
        <input
          ...
          onChange={event => {
            ...
            if (file) handlePick(file, 'reference')
            event.target.value = ''
          }}
        />
      </div>
...
function RatioPicker({ value, onChange }) {
  ...
  const active = ASPECT_RATIOS.find(item => item.value === value) || ASPECT_RATIOS[0]
  ...
  return (
    <div className="relative flex-1" ref={rootRef}>
      <button
        type="button"
        className="w-full flex items-center justify-between gap-3 bg-ink-100 border-none rounded-lg text-sm px-3 py-1.5 outline-none font-medium text-ink-700 hover:bg-ink-200 transition-colors"
        onClick={() => setOpen((v) => !v)}
      >
        <span className="inline-flex items-center gap-2 min-w-0">
          <Icon name={active.icon} size={14} style={active.iconStyle} />
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 z-40 w-[min(340px,calc(100vw-1.5rem))] rounded-xl border border-ink-200 bg-white shadow-2xl overflow-hidden">
          {ASPECT_RATIOS.map(item => (
            <button
              key={item.value}
              type="button"
              className={`w-full flex items-center gap-3 px-3 py-2.5 text-left text-sm hover:bg-ink-50 transition-colors ${value === item.value ? 'bg-mint-50 text-mint-700' : 'text-ink-700'}`}
              onClick={() => {
                onChange(item.value)
                setOpen(false)
              }}
            >
              <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-ink-100 text-ink-700">
                <Icon name={item.icon} size={14} style={item.iconStyle} />
              </span>
              <span className="flex-1 min-w-0">{item.label}</span>
              {value === item.value && <Icon name="check" size={14} className="text-mint-600" />}
            </button>
          ))}
        </div>
      )}
    ...
    <section className="engine-chat-shell h-full flex flex-col max-h-full">
      ...
      <div className="flex items-center justify-between shrink-0 mb-4 px-2">
        ...
        <div className="engine-chat-meta">
          ...
          <span className="engine-chat-credit">{unlimited ? '∞ Unlimited' : 'Text generation is free'}</span>
                ...
                <button type="button" className="creation-prompt-block" onClick={copyPrompt}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {generation.enhanced_prompt
                      ...
                      ? <><span style={{ background: 'var(--accent,#7c3aed)', color: '#fff', borderRadius: 4, fontSize: 10, padding: '1px 6px', fontWeight: 700 }}>âœ¦ AI Enhanced</span> Enhanced Prompt</>

                      : 'Raw Prompt (AI enhancement off)'}

                  </span>

                  <p style={{ fontSize: 12, color: 'var(--text-muted, #888)', marginBottom: 4 }}>

                    <em>Original:</em> {generation.raw_prompt || prompt || 'No prompt stored.'}
                  ...
                  {generation.enhanced_prompt && (
                    ...
                    <p style={{ fontSize: 12 }}>
                      ...
                      <em>Enhanced:</em> {generation.enhanced_prompt}
...
function CreationsGallery({
  ...
  const setAIUsageFocus = useUIStore(s => s.setAIUsageFocus)
...
export default function MintAI() {
  ...
  const aiUsageFocus = useMemo(() => ({
    model_id: selectedModel?.id || null,
    tool_type: currentToolType,
    resolution_tier: activeMode === 'chat' ? null : resolution,
    duration: activeMode === 'video' ? duration : null,
    project_id: projectId || null,
  }), [selectedModel?.id, currentToolType, resolution, activeMode, duration, projectId])

  useEffect(() => {
    setAIUsageFocus(aiUsageFocus)
  }, [aiUsageFocus, setAIUsageFocus])
  ...
  const { data: usageData } = useQuery({
    queryKey: ['ai-usage', aiUsageFocus.model_id || 'any', aiUsageFocus.tool_type || 'any', aiUsageFocus.resolution_tier || 'any', aiUsageFocus.duration || 'any'],
    queryFn: () => aiApi.getUsage(aiUsageFocus).then(res => res.data.data),
    enabled: Boolean(selectedModel) || activeMode === 'chat',
    refetchInterval: 60_000,
  })

  const usageSummary = usageData?.usage || null
  const balance = Number(usageSummary?.mintcoin_balance ?? modelData?.balance ?? 0)
  ...
  const chatGenerations = useMemo(() => {
    ...
    if (!chatHistoryData?.generations) return []

    return chatHistoryData.generations

      .filter(g => g.tool_type === 'text')
  ...
  const uploadMutation = useMutation({
    mutationFn: ({ file, role = 'reference' }) => {
    ...
    onMutate: async ({ file, role = 'reference' }) => {
      ...
      setReferences(current => [...current, optimisticAsset])
      return { tempId, preview_url }
    },
    onSuccess: (asset, _variables, context) => {
      setReferences(current => {
        const next = current.map(ref => {
          if (ref.id !== context?.tempId) return ref
          return {
            ...asset,
            preview_url: asset.preview_url || ref.preview_url,
            upload_state: 'ready',
          }
        })
        if (!next.some(ref => ref.id === asset.id)) {
          next.push({ ...asset, upload_state: 'ready' })
        }
        return next
      })
      if (context?.preview_url) {
        setTimeout(() => URL.revokeObjectURL(context.preview_url), 0)
      }
    },
    onError: (err, _variables, context) => {
      ...
      setReferences(current => current.filter(ref => ref.id !== context?.tempId))
      pushToast?.({ type: 'error', title: err.response?.data?.message || err.message || 'Reference upload failed' })
    },
  })
  ...
  const generateMutation = useMutation({
    ...
    mutationFn: () => {
      ...
      const sharedParameters = {
        ...
        ai_prompt: activeMode === 'chat' ? false : aiPrompt, // NEVER enhance text/chat prompts
      ...
      if (activeMode === 'video') {
        const promptAliases = extractAliases(promptValue)
        const startReference = references.find(ref => ref.reference_role === 'start') || null
        const endReference = references.find(ref => ref.reference_role === 'end') || null
        const videoReferenceAliases = references
          .filter(ref => !['start', 'end'].includes(ref.reference_role || 'reference'))
          .map(ref => ref.alias)
        return aiApi.generateEngineVideo({
          model_id: selectedModel?.id,
          prompt: promptValue,
          session_id: session,
          project_id: projectId || null,
          ...
          reference_aliases: Array.from(new Set([...promptAliases, ...videoReferenceAliases])),
          reference_asset_ids: references.map(ref => ref.id),
          thinking_level: thinking,
        }).then(res => res.data.data)
      }

      // Chat / text â€” no AI prompt enhancement, use generic endpoint
      ...
      return aiApi.generate({
        ...
        parameters: { ...sharedParameters, ai_prompt: false },
  ...
  const selectedUsage = usageSummary?.selected || {}
  const costPerGeneration = activeMode === 'video'
    ? Number(selectedUsage.video_cost ?? cost.cost ?? 0)
    : activeMode === 'chat'
      ? 0
      : Number(selectedUsage.image_cost ?? cost.cost ?? 0)
  const unlimitedGeneration = activeMode !== 'chat' && Boolean(
    activeMode === 'video'
      ? selectedUsage.video_unlimited
      : selectedUsage.image_unlimited
  ) && costPerGeneration <= 0

  const totalCost = activeMode === 'chat'
    ? 0
    : (unlimitedGeneration ? 0 : costPerGeneration * batchCount)
  const generationCostLabel = activeMode === 'chat'
    ? 'Free'
    : (unlimitedGeneration
      ? '∞'
      : (totalCost > 0
        ? `${totalCost.toLocaleString('en-IN')} MintCoin${totalCost === 1 ? '' : 's'}`
        : 'Free'))
  const generationStatusLabel = activeMode === 'chat'
    ? 'Text is free'
    : (unlimitedGeneration
      ? '∞ Unlimited generations'
      : (totalCost > 0
        ? `Uses ${totalCost.toLocaleString('en-IN')} MintCoin${totalCost === 1 ? '' : 's'} • ${Math.max(0, balance - totalCost).toLocaleString('en-IN')} remaining`
        : `Free • ${Math.max(0, balance - totalCost).toLocaleString('en-IN')} remaining`))

  const modeLabel = modeTitle(activeMode)

  const promptReferences = activeMode === 'chat' ? [] : references.filter(ref => activeMode !== 'video' || !['start', 'end'].includes(ref.reference_role || 'reference'))
          ...
          {activeMode === 'chat' ? (
            ...
            <>
              ...
              <ReferencesBlock
                ...
                uploadReference={(file, role) => uploadMutation.mutate({ file, role })}
              ...
              <div className="flex flex-col gap-4">
                ...
                {activeMode === 'video' && (
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-ink-500 w-16 shrink-0">Duration</span>
                    <div className="flex flex-1 gap-1 p-1 bg-ink-100 rounded-lg">
                      {VIDEO_DURATIONS.map(item => (
                        <button
                          key={item.value}
                          type="button"
                          className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all ${duration === item.value ? 'bg-ink-900 text-white shadow-sm' : 'text-ink-600 hover:text-ink-900'}`}
                  ...
                  {['1K', '2K', '4K'].map(tier => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <small className="opacity-60 text-[10px]">{tierMeta.unlimited ? 'âˆž' : tierMeta.cost}</small>
        ...
        {activeMode !== 'chat' && (
          ...
          <div className="p-4 border-t border-ink-100 bg-ink-50/50 flex flex-col gap-3 shrink-0">
            ...
            <button
              ...
              {!generateMutation.isPending && activeMode !== 'chat' && (
    ...
    <article className="creation-card grid">
      ...
      <div className="creation-card-overlay">
        ...
        <div className="creation-bottom-left">
          ...
          <span>{platforms.length ? platforms.join(' â€¢ ') : 'Gallery'}</span>
