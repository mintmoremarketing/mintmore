            ...
            <div key={deal.id} style={{
              ...
              <div className="row between" style={{ marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--ink-500)', marginTop: 3 }}>
                    {deal.job?.category?.name} · {timeAgo(deal.updated_at)}
                  </div>
                </div>
              ...
              <div className="grid-2" style={{ gap: 14, marginBottom: 18 }}>
                <div style={{ padding: 14, background: 'var(--paper-tint)', borderRadius: 'var(--radius-md)', border: '1px solid var(--hairline)' }}>
                  ...
                  <div style={{ fontWeight: 500, fontSize: 14 }}>{deal.client?.full_name || '—'}</div>
                  <div style={{ fontSize: 12, color: 'var(--ink-500)' }}>{deal.client?.email}</div>
                </div>
                <div style={{ padding: 14, background: 'var(--paper-tint)', borderRadius: 'var(--radius-md)', border: '1px solid var(--hairline)' }}>
                  <div style={{ fontSize: 11, color: 'var(--ink-500)', textTransform: 'uppercase', letterSpacing: 0.04, marginBottom: 6 }}>Freelancer</div>
                  <div style={{ fontWeight: 500, fontSize: 14 }}>{deal.freelancer?.full_name || '—'}</div>
                  <div style={{ fontSize: 12, color: 'var(--ink-500)', textTransform: 'capitalize' }}>
                    {deal.freelancer?.freelancer_level}
                  </div>
                </div>
              </div>
              ...
              <div style={{
                ...
                {[
                  ...
                  { label: 'Delivery',       value: `${deal.agreed_days} days` },
                  { label: 'Rounds used',    value: `${deal.current_round} of ${deal.max_rounds}` },
                  { label: 'Client budget',  value: deal.job?.budget_amount ? rupee(deal.job.budget_amount) : 'Open', mono: true },
                ].map(item => (
                  <div key={item.label}>
              ...
              <div className="row" style={{ gap: 10 }}>
                <button
                  ...
                  onClick={() => approveMutation.mutate({ jobId: deal.job_id, note: notes[deal.id] })}
                ...
                <button
                  ...
                  onClick={() => rejectMutation.mutate({ jobId: deal.job_id, note: notes[deal.id] })}
