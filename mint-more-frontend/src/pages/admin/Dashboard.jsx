...
function MarketPriceRangesPanel({ pushToast, queryClient }) {
  ...
  const saveRange = useMutation({
    ...
    onError: err => pushToast({ title: 'Failed', body: err.response?.data?.message || 'Check the min/max values', tone: 'amber', icon: 'x' }),
  })
  ...
  return (
    <div className="card reveal" style={{ padding: 20 }}>
      <div className="row between" style={{ gap: 14, alignItems: 'flex-start', marginBottom: 16 }}>
        <div>
          <div className="h-eyebrow" style={{ marginBottom: 6 }}>Market price ranges</div>
          <div style={{ fontSize: 13, color: 'var(--ink-600)', lineHeight: 1.5 }}>
            These ranges control freelancer offer limits by category and level.
          </div>
        </div>
