...
// Detects whether we're running on a real touch device (phone or tablet)
// — not just the prototype frame switcher in the chrome bar.
// Re-checks on resize, orientation change, pointer-type change, AND via a
// ResizeObserver on <html> (preview iframes often don't fire window.resize).
function useRealDevice() {
  const get = () => {
    if (typeof window === "undefined") return { phone: false, tablet: false, real: false, width: 1280 };
    const w = window.innerWidth || document.documentElement.clientWidth || 1280;
    const coarse = !!(window.matchMedia && window.matchMedia("(pointer: coarse)").matches);
    const phone = w < 760;
    const tablet = !phone && w < 1100 && coarse;
    return { phone, tablet, real: phone || tablet, width: w };
  };
  ...
  React.useEffect(() => {
    ...
    // run once immediately after mount in case the initial paint was at
    // a different size than the laid-out iframe.
    update();
    window.addEventListener("resize", update);
    window.addEventListener("orientationchange", update);
    const mm = window.matchMedia ? window.matchMedia("(pointer: coarse)") : null;
    if (mm) {
...
function App() {
  const device = useRealDevice();
  const [authed, setAuthed] = React.useState(false);
  // User's preview preference — only honoured when NOT on a real touch device.
  // On a real phone/tablet, isMobile/useDeviceFrame are derived from `device`
  // directly so we never lock to a stale initial value.
  const [framePref, setFramePref] = React.useState("desktop"); // desktop | mobile
  ...
  // Derived, not stored — always tracks the current viewport.
  const isMobile = device.real || framePref === "mobile";
  const useDeviceFrame = !device.real && framePref === "mobile";
  const frame = isMobile ? "mobile" : "desktop";
  const setFrame = setFramePref;
  ...
  if (!authed) {
    const auth = <AuthScreen onSignIn={signIn} isMobile={isMobile} />;
    return (
      <>
        {!device.real && (
          <ChromeBar frame={frame} onFrame={setFrame} role={role} onRole={changeRole} />
        )}
        {useDeviceFrame ? <MobileStage>{auth}</MobileStage> : auth}
  ...
  const appContent = (
    <div className={`app ${isMobile ? "mobile" : ""} ${device.real ? "real-device" : ""} ${device.tablet ? "tablet" : ""}`}>
      {!isMobile && <Sidebar route={route} onNav={nav} role={role} />}
      {isMobile && drawerOpen && (
        <div className="mobile-drawer-backdrop" onClick={() => setDrawerOpen(false)}>
          <div className="mobile-drawer" onClick={e => e.stopPropagation()}>
            <Sidebar route={route} onNav={nav} role={role} />
          </div>
        </div>
      )}
  ...
  return (
    <>
      ...
      {useDeviceFrame ? <MobileStage>{appContent}</MobileStage> : appContent}
      <ToastHost toasts={toasts} />
